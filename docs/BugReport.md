...
| BUG-06 | FR-09 | **Critical** | Công thức giảm giá `percent` sai → `discount_amount` âm khổng lồ, `final_amount` > tổng | TC_FR09_D1, TC_FR09_B3 (`discount=-3,600,000`) | server.js:399 | [Link Issue #6](https://github.com/Phuc-215/CI-CD_and_Test-Harness_Engineering/issues/5) |
...
| BUG-11 | FR-04 | High | Cập nhật hồ sơ thiếu trường gây mất dữ liệu (Partial-update Data Loss) | TC_FR04_D11 | server.js:120-128 | [Link Issue #11](https://github.com/Phuc-215/CI-CD_and_Test-Harness_Engineering/issues/12) |
| BUG-12 | FR-09 | **Critical** | Bypass kiểm tra đăng nhập trên Coupon API | TC_FR09_D9 | server.js:416 | [Link Issue #12](https://github.com/Phuc-215/CI-CD_and_Test-Harness_Engineering/issues/13) |
...
### BUG-11 (High) — Cập nhật hồ sơ thiếu trường gây mất dữ liệu (Partial-update Data Loss)
`PUT /api/users/me` ghi đè trực tiếp các trường `name`, `shipping_address`, `phone` bằng các biến giải nén từ `req.body`. Nếu người dùng chỉ gửi một trường (ví dụ: chỉ gửi `phone`), các trường còn lại sẽ mang giá trị `undefined`, khiến SQL gán `NULL` vào DB (xóa sạch dữ liệu cũ). 
**Khắc phục:** Fetch dữ liệu cũ từ DB trước khi `UPDATE` hoặc dùng `COALESCE` trong SQL.

### BUG-12 (Critical) — Bypass kiểm tra đăng nhập (Guest Bypass) trên Coupon API
`POST /api/apply-coupon` bỏ qua kiểm tra `max_uses_per_user` và áp dụng thành công mã giảm giá nếu payload không chứa `user_id` (vào thẳng nhánh `else` của hàm check `user_id` ở dòng 416). Hacker không cần đăng nhập vẫn áp dụng được mã giới hạn!
**Khắc phục:** Bắt buộc phải có `user_id` (hoặc chặn ngay bằng middleware `authenticateToken`) trên route này.

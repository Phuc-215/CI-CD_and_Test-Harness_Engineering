...
[`AI_Triage_User_Guide.md`](../docs/AI_Triage_User_Guide.md) — so you only need to set this up once.
For the Qodo Cover workflow file itself (inputs, triggering, reading results), see
[`Qodo_Cover_User_Guide.md`](./Qodo_Cover_User_Guide.md). Findings from an actual run on this setup
are recorded in [`Qodo_Cover_Evaluation.md`](./Qodo_Cover_Evaluation.md).

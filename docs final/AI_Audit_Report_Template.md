...
| `23127124` | Ngô Huỳnh Gia Thoại | Prepare environments (secrets/variables, GitHub self-hosted runner for LLM triage — done manually, no AI) and convert the GitHub Actions workflow into an equivalent Jenkins pipeline (+ minimal GitLab CI backup) for tool comparison | **Jenkins pipeline** `Jenkinsfile` (5-stage mirror of `ci.yml`: guard, coverage, spec allowed-fail, flaky ×10, web/admin Playwright smoke, mobile Jest smoke — smoke/mobile stages wrapped `catchError`→UNSTABLE); **backup** `.gitlab-ci.yml`; **docs** `docs/User_Guide.md`, `docs/Project_Documentation.md` | Every AI-proposed fix was verified against a real, locally-run Jenkins build (console log pasted back after each run) before being trusted — e.g. a wrong Node-version pin surfaced as a real Vite crash, a missing `catchError` wrap surfaced as a real FAILURE build; final state (build #9 UNSTABLE with full report set) confirmed via the Jenkins API | [prompt_log](Prompt_Log-Thoai.md) |
